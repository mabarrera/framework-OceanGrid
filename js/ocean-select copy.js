...
selectOcean.forEach(function(select, index){
    ...
    if(style){
        ...
        btnSelect.classList.add('btn-default')
    }
...
function optionBtn(arrData,select,btnSelect){ 
    let arrIcon = []
...
function optionSelected(arrData,select,oceanSelect){
    ...
    updateBtn(select,btnSelect) 
}
function updateBtn(select,btnSelect){
    btnSelect.innerHTML = "";    
    let value = select.value    
    let childs = select.childNodes
    let arrData = []
    ...
    arrData.forEach(function(data, i){
        ...
        if(ico !=  undefined){
            let icon = document.createElement('i')
                ...
                btnSelect.append(icon)
        }
        let add = data
        if(i != (cant-1)){
            add = data+','
        }
        btnSelect.append(add)
    })
    if(cant == 0){
        btnSelect.append('Nothing Selected')
    }
}
